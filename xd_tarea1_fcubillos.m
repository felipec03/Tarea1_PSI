...
function y_n = winograd(x, h)
    % Check if h's length is a multiple of 3
    if mod(length(h), 3) ~= 0
        error('h must have length multiple of 3');
    end
    
    h = fliplr(h);

    k = length(h) / 3; % Number of 3-element chunks
    L = length(x) + length(h) - 1; % Final output length
    y_n = zeros(1, L); % Initialize output
    
    for i = 0:k-1
        % Extract i-th 3-element chunk from h (1-based indexing)
        start_idx = 3*i + 1;
        end_idx = 3*(i+1);
        h_chunk = h(start_idx:end_idx);
        
        % Compute convolution using the Winograd method for chunk
        conv_i = winograd_ventana(x, h_chunk);
        
        % Calculate shift position (0-based)
        shift = 3*i;
        
        % Add the shifted convolution result to the output
